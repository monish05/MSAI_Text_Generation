...
def auto_samples_per_epoch(shard_paths):
    return max(100000, sum((count_jsonl_lines(p) for p in shard_paths.values() if p.exists())))

def _load_shards(shard_paths, weights):
    ...
    for name, path in shard_paths.items():
        ...
        weight_list.append(weights.get(name, 0) / max(total_w, 1e-09))
    if not sources:
        raise ValueError("No JSONL shards found.")

    return (sources, rows, weight_list)

def _sample_indices(sources, rows, weight_list, n, seed):
    rng = random.Random(seed)

    return [
        (
            (src := rng.choices(sources, weights=weight_list, k=1)[0]),
            rng.randint(0, len(rows[src]) - 1),
        )
        for _ in range(n)
    ]
    ...
    def __init__(
        ...
        if fixed_indices is not None:
            ...
            self._epoch_indices = _sample_indices(
                self.sources, self.rows, self.weight_list, self.samples_per_epoch, seed
            )
...
def build_fixed_val_indices(shard_paths, weights, n, seed):
