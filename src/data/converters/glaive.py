...
FUNC_RESP_RE = re.compile(
    "FUNCTION RESPONSE:\\s*(.*?)(?=\\n\\nASSISTANT:|\\n\\nUSER:|<\\|endoftext\\|>|$)", re.DOTALL
)

def _parse_tools_from_system(system):
    tools = []
    for block in re.finditer(
        '\\{[^{}]*\\"name\\"\\s*:\\s*\\"[^\\"]+\\"[^{}]*\\}', system, re.DOTALL
    ):
        try:
...
def _convert_chat(system, chat, idx):
    ...
    if not tools and "no access to external functions" in system.lower():
        ...
        if users and assistants:
            ...
            return {
                "id": f"glaive-nfc-{idx}",
                "text": text,
                "meta": {"source": "glaive", "type": "no_function"},
            }
        return None
    users = USER_RE.findall(chat)
    if not users:
    ...
    segments = re.split("(USER:|ASSISTANT:|FUNCTION RESPONSE:)", chat)
    i = 0
    assistant_tool = None
...
def convert_glaive(input_path, out_train, out_val, val_ratio=0.1, limit=0):
