"""Scrape entity names from kiosk Archive (mirrors kiosk backend main.py)."""
...
def extract_names_from_csv(filepath: Path) -> List[str]:
    names: List[str] = []
    try:
...
def extract_names_from_text(filepath: Path) -> List[str]:
    names: List[str] = []
    try:
        with open(filepath, encoding="utf-8") as f:
            for line in f:
...
def load_entity_names(archive_dir: Path) -> List[str]:
