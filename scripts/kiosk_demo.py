...
init()
from src.agent.orchestrator import KioskAgent
...
def main():
    ...
    parser.add_argument(
        "--kiosk-root", type=Path, default=None, help="Path to kiosk repo (or KIOSK_ROOT env)"
    )
    ...
    kiosk_root = args.kiosk_root or Path(
        os.environ.get("KIOSK_ROOT", str(ROOT.parent / "kiosk_vanilla"))
    )
    ...
    (model, tokenizer, device) = load_model_and_tokenizer(
        checkpoint, ROOT / "tokenizer", device=args.device
    )
    agent = KioskAgent(
        model, tokenizer, device, tool_schemas=schemas, kiosk_root=kiosk_root, archive=archive
    )
    ...
    def run_one(q):
        ...
        if (
            tool_call.lm_text
            and result.action_parsed
            and (result.action_parsed.get("action") == "noop")
        ):
