...
init()
from src.data.corpora import convert_corpora
from src.data.kiosk_schemas import SCHEMAS_PATH, export_schemas

from src.data.synthetic import process_kiosk_synthetic

from src.paths import KIOSK_SYNTHETIC_RAW, load_config

from train_tokenizer import train_tokenizer

def main():
    parser = argparse.ArgumentParser(
        description="Preprocess on HPC (split synthetic, corpora, tokenizer)."
    )
    parser.add_argument(
        "--config", type=Path, default=None, help="Config YAML (sets mix_weights, converter_limits)"
    )
    parser.add_argument(
        "--skip-kiosk-split", action="store_true", help="Skip kiosk raw -> processed split"
    )
    ...
    if not args.skip_kiosk_split:
        if raw_path.exists():
            ...
            print(
                f"kiosk split: skipped (no {raw_path}; run generate_synthetic.py on laptop and rsync)"
            )
    else:
